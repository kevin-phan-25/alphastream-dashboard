// app/page.tsx — AlphaStream v200000 Dashboard Compact
'use client';
...
export default function Home() {
  ...
  useEffect(() => { fetchData(); const interval = setInterval(fetchData, 6500); return () => clearInterval(interval); }, []);
  useEffect(() => { logsEndRef.current?.scrollIntoView({ behavior: "smooth" }); }, [data.logs]);

  const forceScan = async () => { setScanning(true); await axios.post(`${BOT_URL}/scan`).catch(()=>{}); setTimeout(()=>setScanning(false),2000); };

  const isCloseWindow = data.marketStatus?.includes("CLOSE");
  const winRate = data.stats?.totalTrades ? ((data.stats?.winningTrades||0)/data.stats.totalTrades*100).toFixed(1) : "—";
  ...
  return (
    <div className="min-h-screen bg-black text-white font-mono text-xs">
      ...
      <main className="pt-12 px-3 max-w-2xl mx-auto space-y-3 pb-12">
        {/* EQUITY + UNREALIZED */}
        ...
        {data.positionsList?.length>0 && (
          <div className="bg-gray-900/95 rounded-xl p-2 border border-cyan-600">
            <div className="text-cyan-400 font-bold text-center text-2xs mb-1">LIVE POSITIONS</div>
        ...
        <div className="bg-black/95 rounded-xl p-2 border border-green-700">
          ...
          <div className="bg-black/80 rounded p-1 h-36 overflow-y-auto font-mono text-2xs">
        ...
        <div className="text-center py-2 text-cyan-400 text-2xs font-bold animate-pulse">
          v200000 • ML BRAIN • SENTIMENT • FIRESTORE • CLOSE 3:45–3:59
        </div>
      </main>
    </div>
  );
