    ...
    return (
      <div className="h-screen bg-black flex items-center justify-center text-red-400">
        <div className="text-center">
          ...
          <button onClick={() => { setLoading(true); fetchCoreData(true); }} className="px-6 py-2 bg-cyan-600 rounded font-bold">RECONNECT</button>
        </div>
      </div>
    );
  }

  // All calculations after early returns
    ...
    <div className="h-screen bg-black text-gray-100 overflow-hidden relative flex flex-col">
      ...
      <header className="shrink-0 bg-black/90 backdrop-blur border-b border-cyan-500/30 px-3 py-2 flex justify-between items-center">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* FIXED: REFRESH button with force sync */}
          <button onClick={() => fetchCoreData(true)} className="px-4 py-1.5 bg-gradient-to-r from-cyan-500 to-purple-600 rounded text-xs font-bold flex items-center gap-1 transition-all hover:brightness-110">
          ...
          <button 
            onClick={forceScan} 
            disabled={scanning}
            className={`px-4 py-1.5 bg-gradient-to-r from-cyan-500 to-purple-600 rounded text-xs font-bold flex items-center gap-1 transition-all ${
              scanning ? 'opacity-70 cursor-not-allowed' : 'hover:brightness-110'
            }`}
          >
            {scanning ? (
