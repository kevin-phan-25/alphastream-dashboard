    ...
    <div className={`min-h-screen ${darkMode ? 'bg-black text-gray-200' : 'bg-gray-50 text-gray-800'} transition-colors`}>
      <div className="max-w-5xl mx-auto p-3">
        ...
        <div className="mb-5">
          ...
          <div className="space-y-2">
            {positions.length > 0 ? positions.map((p: any, i: number) => {
              ...
              return (
                <div key={i} className="bg-gray-900/50 p-2 rounded border border-green-700 flex justify-between items-center text-xs">
                  <div>
            ...
            }) : <div className="text-center text-gray-500 py-6 text-xs">No positions</div>}
          </div>
        </div>
        ...
        <div className="mb-14">
          ...
          <div className="bg-gray-900/50 p-2 rounded text-xs font-mono max-h-48 overflow-y-auto border border-gray-800">
            {logs.length === 0 ? (
              <p className="text-center text-gray-500 py-4">No activity</p>
            ) : (
              logs.slice(-10).reverse().map((l: any, i: number) => (
        ...
        <button
          onClick={forceScan}
          disabled={scanning}
          className={`fixed bottom-3 left-1/2 -translate-x-1/2 w-9/12 max-w-xs py-2.5 rounded-full font-bold text-base flex items-center justify-center gap-2 shadow-lg z-50 transition-all text-xs ${
