...
export default function TradingBotDashboard() {
  ...
  const fetchMLStatus = async () => {
    try {
      ...
      // Extended ML metrics (assuming backend provides these)
  ...
  // ROC AUC Helpers
  const maxAucDisplay = Math.max(auc, 0.5);
          ...
          <div className="relative h-64 w-full bg-black/40 rounded-2xl p-4">
            <svg viewBox="0 0 100 100" className="w-full h-full">
              ...
              {[0, 25, 50, 75, 100].map((x, i) => (
                <line key={i} x1={x} y1="0" x2={x} y2="100" stroke="#27272a" strokeWidth="0.5" />
              ))}
              {/* Diagonal */}
            ...
            <div className="absolute top-6 -left-2 text-xs text-zinc-500 rotate-90 origin-left">
              True Positive Rate
            </div>
          </div>
        </div>

        {/* === NEW: Confusion Matrix with Hover Tooltip === */}
        <div className="glass rounded-3xl p-6 mb-8">
                ...
                {confusionMatrix.flat().map((value, idx) => {
                  ...
                  return (
                    <div 
                      ...
                      onMouseEnter={() => setHoveredMatrixCell({
                        row, 
                        col, 
                        value, 
                        label: matrixLabels[idx]
                      })}
          ...
          <div className="text-center text-xs text-zinc-500 mt-4">Hover cells for details</div>
        </div>
                    ...
                    return (
                      <div key={i} className="flex items-center gap-4 group">
                        ...
                        <div className="flex-1 h-8 bg-zinc-900 rounded-full overflow-hidden relative">
                          <div 
                            className={`h-full transition-all duration-300 ${isPositive ? 'bg-emerald-500' : 'bg-rose-500'}`}
                            style={{ width: `${widthPercent}%`, marginLeft: isPositive ? '0' : `${100 - widthPercent}%` }}
                          />
                        </div>
