...
export default function Home() {
  ...
  const ml = data.ml || { tradesLearned: 0, features: 0 };
      ...
      <main className="pt-14 px-4 max-w-4xl mx-auto space-y-4 pb-20">
        ...
        <div className="grid grid-cols-5 gap-3 text-center">
          <div className="bg-gray-900/80 rounded-lg p-3 border border-purple-700">
            ...
            <div className="text-lg font-bold">{data.positions || 0}/3</div>
          ...
          <div className="bg-gray-900/80 rounded-lg p-3 border border-green-700">
            ...
            <div className="text-lg font-bold text-green-400">{data.winRate || "0"}%</div>
            <div className="text-gray-500 text-xs">WIN</div>
          </div>
          <div className="bg-gray-900/80 rounded-lg p-3 border border-yellow-700">
            <Cpu className="w-5 h-5 mx-auto text-yellow-400 mb-1" />
            <div className="text-lg font-bold text-yellow-400">{ml.tradesLearned || 0}</div>
            <div className="text-gray-500 text-xs">LEARNED</div>
          </div>
          <div className="bg-gray-900/80 rounded-lg p-3 border border-cyan-700">
            <Brain className="w-5 h-5 mx-auto text-cyan-400 mb-1 animate-pulse" />
            <div className="text-lg font-bold text-cyan-400">AI</div>
            ...
            {data.logs?.slice(-30).map((l: string, i: number) => (
              <div key={i} className="py-0.5 border-b border-gray-800 last:border-0">
                {l.includes("ML BRAIN LEARNED") ? (
                  ...
                  <span className="text-cyan-400">{l.split("] ")[1]}</span>
                ) : (
                  l.split("] ")[1]
                )}
              </div>
