...
export default function Home() {
  ...
  // YOUR BOT URL — CHANGE ONLY IF YOU RENAMED THE SERVICE
  ...
  const fetchData = async () => {
    try {
      ...
      console.log("Bot not responding yet — will retry...");
    } finally {
      setLoading(false);
    }
  };
  ...
  const forceScan = async () => {
    setScanning(true);
    try {
      await axios.post("https://alphastream-autopilot-1017433009054.us-east1.run.app/scan");
    } catch {}
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="text-center pt-32">
          ...
          <p className="text-2xl font-bold text-purple-400">WAKING ALPHASTREAM v503...</p>
        </div>
      </div>
    );
  }
      ...
      <main className="pt-20 px-4 max-w-4xl mx-auto space-y-6 pb-32">
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          <div className="bg-gray-900/80 rounded-2xl p-5 text-center border border-yellow-700">
            <p className="text-3xl font-bold text-yellow-400">93.1%</p>
