...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white relative overflow-hidden">
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-purple-700/10 via-gray-200/5 to-cyan-700/10 pointer-events-none" />

      {/* Header */}
      <header className="fixed top-0 inset-x-0 bg-gray-900/90 backdrop-blur border-b border-purple-500/30 px-4 py-3 z-50">
      ...
      <main className="pt-20 px-4 max-w-2xl mx-auto space-y-6 pb-20 relative z-10">
        {error && (
        ...
        {/* Equity */}
        <div className="bg-gray-800/80 rounded-2xl p-6 text-center border border-purple-500/30">
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          <div className="bg-gray-800/80 rounded-xl p-4 text-center border border-cyan-500/30">
          ...
          <div className="bg-gray-800/80 rounded-xl p-4 text-center border border-green-500/30">
          ...
          <div className="bg-gray-800/80 rounded-xl p-4 text-center border border-yellow-500/30">
        ...
        {positions.length > 0 ? (
          <div className="bg-gray-800/80 rounded-2xl p-4 border border-green-500/30">
            ...
            <div className="space-y-2">
              {positions.slice(0, 8).map((p: any, i: number) => (
                <div key={i} className="flex justify-between py-2 px-3 bg-gray-900/50 rounded-lg">
          ...
          <div className="bg-gray-800/60 rounded-2xl p-8 text-center text-gray-500 border border-dashed border-gray-700">
            No open positions
          </div>
        )}

        {/* Brain Status */}
        <div className="bg-gray-800/80 rounded-2xl p-5 border border-purple-500/30">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div>Risk: <span className="text-yellow-400 font-bold">{(config.riskPerTrade * 100 || 0).toFixed(1)}%</span></div>
            <div>Gap: <span className="text-cyan-400 font-bold">{config.minGapPct || 0}%</span></div>
            <div>Trail: <span className="text-orange-400 font-bold">{(config.trailingStopPct * 100 || 0).toFixed(1)}%</span></div>
            <div>Heal: <span className={healMode ? "text-orange-400" : "text-gray-500"}>{healMode ? "ON" : "OFF"}</span></div>
          </div>
        </div>

        {/* Logs */}
        <div className="bg-gray-800/70 rounded-2xl p-4 border border-green-700/30">
          <div className="text-green-400 font-bold mb-2 text-center">LIVE LOGS</div>
          <div className="bg-gray-900/50 rounded-lg p-3 h-32 overflow-y-auto font-mono text-xs">
        ...
        <div className="text-center">
          <button
            ...
            className="px-12 py-4 text-lg font-bold rounded-full bg-gradient-to-r from-purple-500/80 to-cyan-500/80 hover:from-purple-400 hover:to-cyan-400 transition-all duration-500 disabled:opacity-50 border border-purple-700/50 shadow-lg relative z-20"
          >
