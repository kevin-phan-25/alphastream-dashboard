...
export default function Home() {
  ...
  // FIXED: Safe field extraction (handles v34–v38 JSON changes)
  const status = data.status || "OFFLINE";
  const mode = data.mode || (data.dry_mode ? "DRY" : "LIVE");
  const dry_mode = data.dry_mode !== false;
  const positionsCount = data.positions_count ?? data.positions ?? 0;
  const max_pos = data.max_pos ?? data.max_positions ?? 5;
  const equity = data.equity || "$0.00";
  const dailyPnL = data.dailyPnL || "0.00%";
  const tradeLog = data.tradeLog || [];
  const backtest = data.backtest || {};
  const isLive = status === "ONLINE" && !dry_mode;

  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-900 via-black to-pink-900 flex items-center justify-center">
        <div className="text-5xl font-black text-purple-400 animate-pulse">Loading v38.0...</div>
      </div>
    );
  }
      ...
      <main className="pt-32 px-6 pb-20">
        <div className="max-w-6xl mx-auto text-center space-y-16">
          ...
          <div className="flex justify-center">
            <div className={`w-64 h-64 rounded-full flex items-center justify-center border-8 shadow-2xl animate-pulse
              ${isLive ? 'bg-green-600/40 border-green-400' : 'bg-yellow-600/40 border-yellow-400'}`}>
              {isLive ? <Activity className="w-32 h-32 text-green-400" /> : <Rocket className="w-32 h-32 text-yellow-400" />}
            </div>
          </div>

          {/* Stats Grid — FIXED FOR v38 */}
          ...
          {/* Backtest Stats — FIXED FOR v38 */}
          <div className="grid grid-cols-2 md:grid-cols-5 gap-8 max-w-4xl mx-auto">
            {[
              { label: "Total Trades", value: totalTrades, color: "text-purple-400" },
              { label: "Win Rate", value: winRate, color: "text-cyan-400" },
              { label: "Wins", value: backtest.wins || 0, color: "text-green-400" },
              { label: "Losses", value: backtest.losses || 0, color: "text-red-400" },
              { label: "Net P&L", value: backtest.totalPnL ? `$${backtest.totalPnL.toFixed(2)}` : "$0.00", color: backtest.totalPnL >= 0 ? "text-green-400" : "text-red-400" },
            ].map((stat) => (
            ...
            <div className="bg-white/5 backdrop-blur-xl rounded-3xl border border-white/20 p-8 max-h-96 overflow-y-auto">
              {(!tradeLog || tradeLog.length === 0) ? (
                ...
                <div className="space-y-5">
                  {tradeLog.slice().reverse().map((t: any) => (
