// src/app/page.tsx — FINAL 2025 MODERN DASHBOARD (100% WORKING)
'use client';
...
export default function Home() {
  ...
  const [scanning, setScanning] = useState(false); // ← FIXED: Removed the "-"
    ...
    <div className="min-h-screen bg-black text-white relative overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-6 py-24">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-4xl mt-4 text-gray-400 font-light">{data.version || "v29.0"}</p>
        </div>

        {/* Status Circle */}
        <div className="flex justify-center mb-20">
          <div className={`relative w-96 h-96 rounded-full flex items-center justify-center
            ${isLive ? 'bg-green-500/20 border-12 border-green-400 shadow-2xl shadow-green-500/60' : 'bg-yellow-500/20 border-12 border-yellow-400 shadow-2xl shadow-yellow-500/60'}
            animate-pulse`}>
            {isLive ? (
        ...
        {data.dry_mode && (
          <div className="max-w-4xl mx-auto bg-gradient-to-r from-yellow-600/30 to-orange-600/30 border-4 border-yellow-500 rounded-3xl p-12 text-center backdrop-blur-xl">
            ...
            <p className="text-3xl">Set <code className="bg-black/60 px-6 py-3 rounded-xl">DRY_MODE=false</code> in Cloud Run → Go LIVE</p>
          </div>
        )}
