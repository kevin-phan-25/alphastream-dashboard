...
export default function Home() {
  ...
  const unrealized = data.unrealized ? (data.unrealized >= 0 ? `+$${data.unrealized.toLocaleString()}` : `-$${Math.abs(data.unrealized).toLocaleString()}`) : "$0";
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-black to-pink-900 text-white pb-32">
      ...
      <main className="pt-40 px-6 max-w-7xl mx-auto space-y-12">
        ...
        {btTrades > 0 && (
          <div className="bg-black/70 backdrop-blur-2xl rounded-3xl p-12 border-4 border-cyan-500 shadow-2xl">
            ...
            <div className="grid grid-cols-3 md:grid-cols-6 gap-8 text-center">
              <div><p className="text-gray-400">Trades</p><p className="text-5xl font-bold text-white">{btTrades}</p></div>
              <div><p className="text-gray-400">Win Rate</p><p className="text-5xl font-bold text-green-400">{btWinRate}%</p></div>
              <div><p className="text-gray-400">P/F</p><p className="text-5xl font-bold text-cyan-400">{btProfitFactor}</p></div>
              <div><p className="text-gray-400">Net P&L</p><p className={`text-5xl font-bold ${btTotalPnL >= 0 ? 'text-green-400' : 'text-red-400'}`}>${btTotalPnL >= 0 ? '+' : ''}{Math.abs(btTotalPnL).toLocaleString()}</p></div>
              <div><p className="text-gray-400">Max DD</p><p className="text-5xl font-bold text-orange-400">{btMaxDD}%</p></div>
              <div><p className="text-gray-400">Best</p><p className="text-5xl font-bold text-yellow-400">+${btBestTrade.toLocaleString()}</p></div>
            </div>
          </div>
        )}
          ...
          <div className="bg-black/50 backdrop-blur-2xl rounded-3xl p-12 border-4 border-yellow-500 shadow-2xl">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-8">
              {rockets.map((r: string, i: number) => {
                ...
                const float = r.includes('(') ? r.split('(')[1].replace(')', '') : '';
                return (
                  <div key={i} className="bg-gradient-to-br from-purple-700 to-pink-800 rounded-2xl p-6 text-center hover:scale-110 transition">
                    ...
                    {float && <p className="text-lg text-gray-300 mt-1">{float}</p>}
                  </div>
                );
              })}
            </div>
        ...
        <div className="grid md:grid-cols-2 gap-12 max-w-5xl mx-auto pt-8">
          ...
          <button onClick={triggerBacktest} disabled={backtesting}
