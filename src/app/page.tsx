...
export default function Home() {
  ...
  // FIX: Parse $ strings → real numbers
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-black to-pink-900 text-white pb-32">
      ...
      <main className="pt-40 px-6 max-w-7xl mx-auto space-y-12">
        ...
        {/* MAIN STATS — NOW SHOWS CORRECT $101,552 */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          <div className="bg-white/5 backdrop-blur-xl rounded-3xl p-10 text-center border-2 border-cyan-500/60 hover:scale-105 transition">
            ...
            <p className="text-6xl font-black text-cyan-300">{equity}</p>
            <p className="text-xl text-gray-300">Equity</p>
          </div>

          <div className="bg-white/5 backdrop-blur-xl rounded-3xl p-10 text-center border-2 ${rawUnrealized >= 0 ? 'border-green-500/60' : 'border-red-500/60'} hover:scale-105 transition">
            <TrendingUp className={`w-20 h-20 mx-auto mb-4 ${rawUnrealized >= 0 ? 'text-green-400' : 'text-red-400'}`} />
            <p className={`text-6xl font-black ${rawUnrealized >= 0 ? 'text-green-400' : 'text-red-400'}`}>
              {unrealized}
            </p>
          ...
          <div className="bg-white/5 backdrop-blur-xl rounded-3xl p-10 text-center border-2 border-purple-500/60 hover:scale-105 transition">
            ...
            <p className="text-6xl font-black text-purple-300">{positionsCount}</p>
          ...
          <div className="bg-white/5 backdrop-blur-xl rounded-3xl p-10 text-center border-2 border-orange-500/60 hover:scale-105 transition">
            ...
            <p className="text-6xl font-black text-orange-300">{rockets.length}</p>
          ...
          <div className="bg-black/50 backdrop-blur-2xl rounded-3xl p-12 border-4 border-yellow-500 shadow-2xl">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-8">
              {rockets.map((r: string, i: number) => {
                ...
                return (
                  <div key={i} className="bg-gradient-to-br from-purple-700 to-pink-800 rounded-2xl p-8 text-center hover:scale-110 transition shadow-2xl">
                    <p className="text-5xl font-black">{sym}</p>
                    <p className="text-4xl text-green-400">+{pct}</p>
                    {floatPart && <p className="text-lg text-gray-300 mt-2">{floatPart}</p>}
                  </div>
                );
              })}
            </div>
        ...
        {/* BUTTONS — BIGGER, CENTERED, WORKING */}
        <div className="grid md:grid-cols-2 gap-12 max-w-5xl mx-auto">
          <button
            onClick={triggerScan}
            disabled={scanning}
            className="py-24 text-7xl font-black rounded-3xl bg-gradient-to-r from-cyan-600 to-purple-700 hover:from-cyan-500 hover:to-purple-600 disabled:opacity-50 transition-all shadow-2xl border-8 border-cyan-400 flex items-center justify-center gap-12"
          >
            <RefreshCw className={`w-32 h-32 ${scanning ? 'animate-spin' : ''}`} />
          ...
          <button
            ...
            className="py-24 text-7xl font-black rounded-3xl bg-gradient-to-r from-orange-600 to-red-700 hover:from-orange-500 hover:to-red-600 disabled:opacity-50 transition-all shadow-2xl border-8 border-orange-400 flex items-center justify-center gap-12"
          >
            <BarChart3 className={`w-32 h-32 ${backtesting ? 'animate-spin' : ''}`} />
